...
const addExclamation = (arr) => {
  let loudArr = [];
  arr.forEach(value => loudArr.push(value + '!'));
  return loudArr;
};
...
const allUpperCase = (arr) => {
  let capslockArr = [];
  arr.forEach(value => capslockArr.push(value.toUpperCase()));
  return capslockArr;
};
...
const speaker = (words, callback) => {
  let speakerArr = [];
  words.forEach(value => speakerArr.push(callback(value)));
  return speakerArr;
};
...
const createList = (availableItems) => {
  let availableList = [];
  availableItems.forEach(value => {if (value.available === true){availableList.push(value.name);}});
