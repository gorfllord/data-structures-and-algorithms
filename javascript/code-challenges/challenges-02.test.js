...
const raisedToTheThird = (arr) => {
  let cubedNumbers = [];
  arr.forEach(value => cubedNumbers.push(Math.pow(value, 3)));
  return cubedNumbers;
};
...
const addOne = (arr) => {
  let addedOne = arr.map(function(n, i){
    return n + 1;
  });
  return addedOne;
};
...
const addQuestion = (arr) => {
  let addedQuestion = arr.map(function(n, i){
    return n + '?';
  });
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    forLooped.push(Math.pow(2, arr[i]));
  }
  return forLooped;
};
...
const forEachTwoToThe = (arr) => {
  let forEached = [];
  arr.forEach(n => forEached.push(Math.pow(2, n)));
  return forEached;
};
...
const mapTwoToThe = (arr) => {
  let mapPowered = arr.map(function(n, i) {
    return Math.pow(2, n);
  });
  return mapPowered;
};
...
const charCode = (arr) => {
  let charCoded = arr.map(function(n, i) {
...
const evenOdd = (arr) => {
  let evenOddArray = arr.map((n, i) => {
    if ((n / 2) % 1 === 0) {
      return 'even';
    } else if ((n / 2) % 1 !== 0 && Number.isInteger(n)) {
      return 'odd';
    } else {
      return 'N/A';
    };
...
const extractAbilities = (arr) => {
  let snorlaxAbilities = arr.map((n, i) => {
    return n.ability.name;
  });
  return snorlaxAbilities;
};
...
const extractStats = (arr) => {
  let snorlaxStatResults = arr.map((n, i) => {
    let result = {
      name: n.stat.name,
      total: (n.effort + n.baseStat)};
    return result;
  });
  return snorlaxStatResults;
};
